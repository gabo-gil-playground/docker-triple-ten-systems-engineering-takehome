...
A `scan` job runs on every push and PR, installing Trivy via its official install script and scanning each of the three Docker images (`producer`, `worker`, `payments`) for CRITICAL severity vulnerabilities with fixes available (`--ignore-unfixed`). The job fails on any finding (`--exit-code 1`), blocking merge until the vulnerability is addressed. HIGH severity findings are reported but do not block due to expected noise from base image system libraries. A complementary `hadolint` step lints the three Dockerfiles against best practices: pinned base image digests, non-root USER directives, COPY ordering for cache efficiency, and avoidance of `latest` tags. The images inherit from `python:3.12-slim`, which is regularly patched upstream, so scan noise is expected to be low. This gate prevents vulnerable images and Dockerfile drift from reaching production without adding GitHub Actions marketplace dependencies.
